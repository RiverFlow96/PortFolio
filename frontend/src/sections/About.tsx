  ...
  return (
    <section id="about" ref={sectionRef} className="section-padding relative">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-[1fr_1.5fr] gap-12 lg:gap-16">
          ...
          <div className="space-y-8">
            ...
            <div className="relative inline-block">
              <div className="w-32 h-32 rounded-full overflow-hidden border-2 border-[var(--border)]">
                <img 
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%2314b8a6'%3E%3Cpath d='M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z'/%3E%3C/svg%3E";
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-4 text-[var(--text-secondary)] leading-relaxed">
              {profile.bio.map((paragraph, i) => (
                <p key={i} className={isVisible ? 'animate-slide-up' : 'opacity-0'} style={{ animationDelay: `${i * 0.15}s` }}>
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            <div className="flex gap-3">
              {profile.social?.map((social: { label: string; href: string; icon: string }) => (
                <a
                  ...
                  className="w-10 h-10 rounded-lg border border-[var(--border)] flex items-center justify-center text-[var(--text-muted)] hover:border-[var(--accent)] hover:text-[var(--accent)] transition-all duration-300"
                >
                  {social.icon === 'github' && (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {stats.map((item, i) => (
              <div
                key={item.label}
                className={`p-6 rounded-xl border border-[var(--border)] hover:border-[var(--border-hover)] transition-all duration-300 ${isVisible ? 'animate-scale-in' : 'opacity-0'}`}
        ...
        {/* Philosophy - minimal card */}
        <div className="mt-16 p-6 rounded-xl border border-[var(--border)] bg-[var(--bg-secondary)]">
