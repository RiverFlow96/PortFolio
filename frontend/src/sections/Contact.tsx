...
export function Contact(): JSX.Element {
  ...
  const validateForm = (): boolean => {
    ...
    else if (formData.name.trim().length < 2) newErrors.name = "Minimo 2 caracteres";
    if (!formData.email.trim()) newErrors.email = "El email es requerido";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) newErrors.email = "Email invalido";
    if (!formData.message.trim()) newErrors.message = "El mensaje es requerido";
    else if (formData.message.trim().length < 10) newErrors.message = "Minimo 10 caracteres";
    setErrors(newErrors);
    ...
    <section id="contact" className="section-padding-lg relative" aria-labelledby="contact-heading">
      ...
      <div className="container-main relative z-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-[var(--text-muted)] font-mono text-sm tracking-wider">contact</span>
        </div>

        <h2 id="contact-heading" className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--text-primary)] mb-6 tracking-tight">
          ¿Habl<span className="text-gradient">amos</span>?
        </h2>

        <p className="text-[var(--text-secondary)] mb-16 max-w-xl text-lg">
          Estoy abierto a nuevas oportunidades, colaboraciones y proyectos desafiantes. Escribeme.
        </p>

        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
          <div className="space-y-8">
            <h3 className="text-xl font-semibold text-[var(--text-primary)]">Conectemos</h3>
              ...
              {socials.map((social: { label: string; href: string; icon: string }) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[var(--text-secondary)] group-hover:text-[var(--text-primary)] transition-colors font-medium">{social.label}</span>
                  </div>
            ...
            <div className="p-6 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border)]">
              <p className="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wider mb-2">Email directo</p>
              <a href={`mailto:${contactConfig.email}`} className="text-lg text-[var(--accent)] hover:text-[var(--accent-hover)] transition-colors font-medium">{contactConfig.email}</a>
            </div>
          </div>

          <div className="glass rounded-3xl p-8 hover:border-[var(--border-hover)] transition-all duration-500">
            <h3 className="text-xl font-semibold text-[var(--text-primary)] mb-6">Envia un mensaje</h3>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                ...
                {status === "idle" ? "Enviar mensaje" : status === "loading" ? "Enviando..." : status === "success" ? contactConfig.successMessage : contactConfig.errorMessage}
              </button>
            </form>
          </div>
        </div>
